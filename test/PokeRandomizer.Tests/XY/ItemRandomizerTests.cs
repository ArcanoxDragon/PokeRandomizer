	...
	public class ItemRandomizerTests
	{
		...
		public async Task LoadRandomizer()
		{
			...
			RandomizerConfig randConfig = new RandomizerConfig {
				EggMoves = { },
				Encounters = { LevelMultiplier = 1.25m, TypeThemedAreas = true },
				Learnsets = { AtLeast4Moves = true, RandomizeLevels = true },
				OverworldItems = { },
				PokemonInfo = { },
				Starters = { StartersOnly = false },
				Trainers = { FriendKeepsStarter = true, LevelMultiplier = 1.3m, TypeThemed = true }
			};
		...
		public async Task TestItemRandomizer()
		{
			...
			for (var i = 0; i < 206; i++)
			{
				...
				await TestContext.Progress.WriteLineAsync($"Script {scriptId:X4} has item {itemName} x{quantity}, changing to {newItemName}");
			...
			await File.WriteAllBytesAsync("old.bin", fldItemAmx.Data);
			await File.WriteAllBytesAsync("new.bin", newData);
