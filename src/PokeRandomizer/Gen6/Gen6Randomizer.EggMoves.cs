...
namespace PokeRandomizer.Gen6
{
	public partial class Gen6Randomizer
	{
		...
		{
			var config = ValidateAndGetConfig().EggMoves;

			if (!config.RandomizeEggMoves)
				return;

			await LogAsync($"======== Beginning Egg Move randomization ========{Environment.NewLine}");
			progressNotifier?.NotifyUpdate(ProgressUpdate.StatusOnly("Randomizing egg moves..."));

			var eggMovesList = await Game.GetEggMoves();
			...
			for (var i = 0; i < eggMovesList.Length; i++)
			{
				...
				for (var m = 0; m < eggMoves.Count; m++)
				{
					...
					await LogAsync($" - {moveNames[eggMoves.Moves[m]]}");
				}
			...
			await Game.SaveEggMoves(eggMovesList);
			await LogAsync($"======== Finished Egg Move randomization ========{Environment.NewLine}");
		}
	}
}