...
namespace PokeRandomizer.Tests.ORAS.Rewrite
{
	...
	public partial class RomFS
	{
		...
		{
			var (outPath, outDir) = GetAndCreateGarcPath(garc);
			...
			await using var origFs = new FileStream(Path.Combine(outDir, $"{fname}.orig"), FileMode.Create, FileAccess.Write, FileShare.None);
			...
			await TestContext.Out.WriteLineAsync($"GARC {garc.Reference.RomFsPath} ({garc.Reference.Name}):");
			await TestContext.Out.WriteLineAsync($"\tOld hash: {string.Join("", hashOriginal.Select(d => d.ToString("X2")))}");
			await TestContext.Out.WriteLineAsync($"\tNew hash: {string.Join("", hashNew.Select(d => d.ToString("X2")))}");
		...
		public async Task RewritePokemonInfo()
		{
			...
			await TestGarcStructure(garcPokeInfo, async () => {
		...
		public async Task RewriteLearnsets()
		{
			...
			await TestGarcStructure(garcLearnsets, async () => {
		...
		public async Task RewriteMoves()
		{
			...
			await TestGarcStructure(garcMoves, async () => {
		...
		public async Task RewriteEncounters()
		{
			...
			await TestGarcStructure(garcEncounters, async () => {
		...
		public async Task RewriteEggMoves()
		{
			...
			await TestGarcStructure(garcEggMoves, async () => {
		...
		public async Task RewriteTrainers()
		{
			...
			await TestGarcStructure(garcTrainerData, () => TestGarcStructure(garcTrainerPoke, async () => {
