...
namespace PokeRandomizer.Tests.ORAS
{
	...
	public class TextFileTests
	{
		...
		public async Task TestEachLine()
		{
			...
			{
				await TestContext.Progress.WriteLineAsync($"Testing line {i}...");
				...
				if (i < tf0.Lines.Count - 1)
				{
					...
					await TestContext.Progress.WriteLineAsync($"               Length: {lineInfo.Length * 2}");
					await TestContext.Progress.WriteLineAsync($"              Padding: {padding}");
					await TestContext.Progress.WriteLineAsync($"                 Flag: {lineInfo.Flag}");
					await TestContext.Progress.WriteLineAsync($"    Length w/ padding: {lineInfo.Length * 2 + padding}");
					await TestContext.Progress.WriteLineAsync($"         Char Len % 2: {lineInfo.Length % 2}");
					await TestContext.Progress.WriteLineAsync($"       Len w/ pad % 2: {( lineInfo.Length + ( padding / 2 ) ) % 2}");
				}
