	...
	public partial class RomFS
	{
		...
		public async Task RewriteGameText()
		{
			...
			for (int i = 0; i < gameText.Length; i++)
			{
				...
				await File.WriteAllLinesAsync(path, gameText[i].Lines);
			}
			...
			await TestContext.Progress.WriteLineAsync($"Testing {gameText.Length} text files...");
			await TestContext.Progress.WriteLineAsync($"S{"-".Repeat(ProgressBarWidth - 2)}E");

			void UpdateProgress()
			{
				int intProgress = (int) Math.Ceiling(progress * ProgressBarWidth);
			...
			await TestGarcStructure(garcGameText, async () => {
				for (int i = 0; i < gameText.Length; i++)
				{
					int file = i;
					...
					await File.WriteAllLinesAsync(path, test.Lines, testEncoding);
		...
		{
			await using var fs = new FileStream(Path.Combine(this.path, "GameText.diff.txt"), FileMode.Create, FileAccess.Write, FileShare.None);
