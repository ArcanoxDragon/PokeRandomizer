...
namespace PokeRandomizer.Gen6.ORAS
{
	public partial class OrasRandomizer
	{
		...
		{
			var config = ValidateAndGetConfig().Encounters;

			if (!config.RandomizeEncounters)
				return;

			await LogAsync($"======== Beginning Encounter randomization ========{Environment.NewLine}");
			...
			var encounterData = await Game.GetEncounterData();
			...
			foreach (var (iEncounter, encounterZone) in encounterZones.Pairs())
			{
				...
				void UpdateChoiceList(IList<SpeciesType> choices)
				{
					...
					while (uniqueList.Count < MaxUniqueSpecies)
						...
						uniqueList.Add(GetRandomSpecies(taskRandom, choices.Except(uniqueList)));
				...
				if (areaType == default || config.TypePerSubArea)
					await LogAsync($"Randomizing zone: {name}");
				else
					await LogAsync($"Randomizing zone: {name} ({areaType.Name}-type)");
				...
				foreach (var (iSubZone, subZoneArray) in subZoneArrays.Pairs())
				{
					...
					if (areaType != default && config.TypePerSubArea)
						await LogAsync($"  Sub-zone {iSubZone + 1} ({areaType.Name}-type):");
					else
						await LogAsync($"  Sub-zone {iSubZone + 1}:");
					...
					foreach (var (iEntry, entry) in subZoneArray.Where(entry => entry.Species != Species.Egg.Id).Pairs())
					{
						...
						else
						{
							if (config.ProperHordes && isHordeEncounter)
							{
								...
								if (iEntry == hordeSlot)
								{
									entry.Species = (ushort) GetRandomSpecies(taskRandom, speciesChoose).Id;
								}
								else
								{
									if (hordeSpecies < 0)
										hordeSpecies = GetRandomSpecies(taskRandom, speciesChoose).Id;
						...
						await LogAsync($"    Entry: {speciesNames[entry.Species]}, levels {entry.MinLevel} to {entry.MaxLevel}");
					}
			...
			await Game.SaveEncounterData(encounterZones);
			await LogAsync($"======== Finished Encounter randomization ========{Environment.NewLine}");
		}
	}
}