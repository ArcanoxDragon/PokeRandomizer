...
namespace PokeRandomizer.Tests.RandomizerTests
{
	...
	public class TempTests
	{
		...
		protected async Task<(GameConfig, RandomizerConfig)> GetGameAndConfigAsync()
		{
			...
			var configJson = await File.ReadAllTextAsync(@"D:\Users\Arcanox\Documents\ROMs\3DS\OmegaRuby\Random Preferred.json");
		...
		{
			var (game, config) = await GetGameAndConfigAsync();
		...
		{
			var (game, _) = await GetGameAndConfigAsync();
		...
		{
			var (game, _ /*config*/) = await GetGameAndConfigAsync();
			var whitelist = await GetOriginalNullEncounters();
		...
		public async Task DumpCatchRates()
		{
			...
			await File.AppendAllTextAsync(Path, "NationalDexId,Name,CatchRateDec,CatchRateHex");
			...
			foreach (var (iPokeInfo, pokeInfo) in pokeInfoTable.Pairs())
			{
				...
				await File.AppendAllTextAsync(Path, $"{natDex:000},{name},{pokeInfo.CatchRate:000},0x{pokeInfo.CatchRate:X2}");
				await File.AppendAllTextAsync(Path, Environment.NewLine);
			}
		}
	}
