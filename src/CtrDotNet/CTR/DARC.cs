...
namespace CtrDotNet.CTR
{
	public class Darc
	{
		...
		internal static Darc GetDarc(string folderName)
		{
...

			{
				...
				foreach (string folder in Directory.GetDirectories(folderName))
				{
					...
					entryList.Add(new FileTableEntry { DataOffset = 1, DataLength = (uint) ( files.Length + entryList.Count ), IsFolder = true, NameOffset = nameOffset });
					...
					foreach (string file in files)
					{
						...
						entryList.Add(new FileTableEntry { DataOffset = (uint) data.Length, DataLength = (uint) fi.Length, IsFolder = false, NameOffset = nameOffset });
		...
		internal static bool Darc2Files(string path, string folderName)
		{
			try
			{
				return Darc2Files(File.ReadAllBytes(path), folderName);
			}
			catch (Exception)
			{
				return false;
		...
		internal static bool InsertFile(ref Darc orig, int index, string path)
		{
			try
			{
				return InsertFile(ref orig, index, File.ReadAllBytes(path));
			}
			catch (Exception)
			{
				return false;
