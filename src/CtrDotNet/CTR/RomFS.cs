...
namespace CtrDotNet.CTR
{
	public class RomFS
	{
		...
		internal static void BuildRomFS(string infile, string outfile)
		{
			...
			{
				romFiles[i] = new RomfsFile { Offset = Out[i].Offset, PathName = Out[i].FilePath.Replace(Path.GetFullPath(RootDir), "").Replace("\\", "/"), FullName = Out[i].FilePath, Size = Out[i].Size };
			}

			using (MemoryStream memoryStream = new MemoryStream())
			{
				BuildRomFSHeader(memoryStream, romFiles, RootDir);
			...
			try
			{
				...
				for (int i = ivfc.Levels.Length - 1; i >= 0; i--)
				{
					...
					if (i > 1)
						...
						cOfs = (long) Align(headerLen, PaddingAlign);
				}

				outFileStream.Seek(0, SeekOrigin.Begin);
				uint superBlockLen = (uint) Align(masterHashLen + 0x60, mediaUnitSize);
		...
		internal static void BuildRomFSHeader(MemoryStream romfsStream, RomfsFile[] entries, string dir)
		{
			...
			WriteMetaDataToStream(metaData, romfsStream);
		}
		...
		internal static void CalcRomfsSize(RomfsMetaData metaData)
		{
			...
			CalcDirSize(metaData, rootDi);

			metaData.MDirHashTableEntry = GetHashTableEntryCount(metaData.DirNum);
			metaData.MFileHashTableEntry = GetHashTableEntryCount(metaData.FileNum);

			uint metaDataSize = (uint) Align(0x28 + metaData.MDirHashTableEntry * 4 + metaData.MDirTableLen + metaData.MFileHashTableEntry * 4 + metaData.MFileTableLen, PaddingAlign);
		...
		internal static void CalcDirSize(RomfsMetaData metaData, DirectoryInfo dir)
		{
			if (metaData.MDirTableLen == 0)
				...
				metaData.MDirTableLen += 0x18 + (uint) Align((ulong) dir.Name.Length * 2, 4);

			FileInfo[] files = dir.GetFiles();
			foreach (FileInfo t in files)
				metaData.MFileTableLen += 0x20 + (uint) Align((ulong) t.Name.Length * 2, 4);
		...
		internal static void PopulateRomfs(RomfsMetaData metaData, RomfsFile[] entries)
		{
			...
			AddDir(metaData, new DirectoryInfo(RootDir), 0, RomfsUnusedEntry);
		...
		internal static void AddDir(RomfsMetaData metaData, DirectoryInfo dir, uint parent, uint sibling, bool doSubs)
		{
			...
			if (!doSubs)
			{
				...
				metaData.DirTableLen += currentDir == 0
											? 0x18
											: 0x18 + (uint) Align((ulong) dir.Name.Length * 2, 4);
